...
void graph_GetColorByVelocity(lb_float velocity, lb_float * r, lb_float * g,
			      lb_float * b)
{
	lb_float max_color_1 = 217.0f, max_color_2 = 232.0f, max_color_3 =
	    255.0f, min_color_1 = 0, min_color_2 = 2.0f, min_color_3 =
	    6.0f, clm = 255.0f;
 ...
 */
void GRAPH_RedrawSmooth(const LB_Lattice_p lattice,
			const EXTOBJ_obj_set_p obj_set)
{
	...
	for (int ypos = 1; ypos < lattice->countY; ++ypos) {
		for (int xpos = 1; xpos < lattice->countX; ++xpos) {
			...
			int r[4] = {
				BASE_GetIdxByPos(lattice, xpos - 1, ypos - 1,
						 0),
			 ...
			 const EXTOBJ_obj_set_p obj_set)
{
	...
	for (i = 0; i < nodes_cnt; ++i) {
		...
		v_sum = sqrt(ch_vector->x * ch_vector->x +
 ...
 */
void GRAPH_RedrawSolid(const LB_Lattice_p lattice,
		       const EXTOBJ_obj_set_p obj_set)
{
	LB3D_p ch_vector = lattice->velocities;
	lb_float minv = 1000000, maxv = 0, avgv = 0;
	...
	for (int ypos = 0; ypos < lattice->countY; ypos++) {
		for (int xpos = 0; xpos < lattice->countX; xpos++) {
			...
			lb_float dx =
			    lattice->velocities[xpos * ypos].x > 0 ? d : -d;
			dx *= lattice->sizeX / lattice->countX;
			lb_float dy =
			    lattice->velocities[xpos * ypos].y > 0 ? d : -d;
	...
	for (int ypos = 0; ypos < lattice->countY - 1; ypos++) {
		for (int xpos = 0; xpos < lattice->countX - 1; xpos++) {
			...
			glColor3f(red, green, blue);
			x = ((lb_float) xpos +
			     1.0 / 3.0) * lattice->sizeX / lattice->countX;
			y = ypos * lattice->sizeY / lattice->countY;
			glVertex2f(x, y);
			...
			glColor3f(red, green, blue);
			x = ((lb_float) xpos +
			     2.0 / 3.0) * lattice->sizeX / lattice->countX;
			y = ypos * lattice->sizeY / lattice->countY;
			glVertex2f(x, y);

			lb_float sq_4 = (2.0 * sq1 + sq3) / 3.0;
			graph_GetColorByVelocity(sq_4 / new_max, &red, &green,
						 &blue);
			glColor3f(red, green, blue);
			x = (xpos) * lattice->sizeX / lattice->countX;
			y = ((lb_float) ypos +
			     1.0 / 3.0) * lattice->sizeY / lattice->countY;
			glVertex2f(x, y);
			...
			x = (xpos) * lattice->sizeX / lattice->countX;
			y = ((lb_float) ypos +
			     2.0 / 3.0) * lattice->sizeY / lattice->countY;
			glVertex2f(x, y);
 ...
 */
void GRAPH_RenderWorld(const LB_Lattice_p lattice,
		       const EXTOBJ_obj_set_p obj_set)
{
 ...
 * 
 */
void GRAPH_DrawButton(lb_float x, lb_float y, lb_float width, lb_float height,
		      char *text)
{
 ...
 * 
 */
void GRAPH_DrawLabel(lb_float x, lb_float y, lb_float width, lb_float height,
		     char *text)
{
	glColor3f(0.1f, 0.1f, 0.1f);
	glBegin(GL_QUADS);
